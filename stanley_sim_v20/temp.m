...
path_lx = [0:12.5/100:12.5 - 12.5/100,12.5* ones(1,51)]; % left x
path_ly = [5 * ones(1,100), 5:-5/50:0 ]; % left y
path_rx = [0:15/100:15-15/100 , 15*ones(1,51)]; % right x
path_ry = [10.5 * ones(1,100), 10.5:-10.5/50:0 ]; % right y
%% target path (2/2): waypoint generation
for i = 1:1:length(path_lx) path_cx(i) = (path_lx(i)+path_rx(i))/2;
...
for i = 0:dt:sim_time
%% Find closest pointfunction 

idx = find_closest_point(x_v+dfx ,y_v+dfy ,path_x ,path_y);
look_ahead_point = idx + look_ahead_idx;
cx1 = path_x(look_ahead_point);
cy1 = path_y(look_ahead_point);
if (look_ahead_point == length(path_x)) % Check end condition 
break;
else
cx2 = path_x(look_ahead_point+1) + esp;
cy2 = path_y(look_ahead_point+1) + esp;
end 
    %% Calculation x 
 ...
 %% Calculation alpha
    ...
    %% Update vehicle 
theta=theta_vehicle ;
x_v = x_v + v * cos( theta + cmd_sta ) * dt;
y_v = y_v + v * sin( theta + cmd_sta ) * dt;
theta = theta + v * tan(cmd_sta)/L * dt;


updatedFx = [shape_fx(1) * cos(theta+cmd_sta) - shape_fy(1) * sin(theta+cmd_sta) + dfx + x_v
shape_fx(2) * cos(theta+cmd_sta) - shape_fy(2) * sin(theta+cmd_sta) + dfx + x_v
shape_fx(3) * cos(theta+cmd_sta) - shape_fy(3) * sin(theta+cmd_sta) + dfx + x_v
shape_fx(4) * cos(theta+cmd_sta) - shape_fy(4) * sin(theta+cmd_sta) + dfx + x_v];
updatedFy = [shape_fx(1) * sin(theta+cmd_sta) + shape_fy(1) * cos(theta+cmd_sta) + dfy + y_v
shape_fx(2) * sin(theta+cmd_sta) + shape_fy(2) * cos(theta+cmd_sta) + dfy + y_v
shape_fx(3) * sin(theta+cmd_sta) + shape_fy(3) * cos(theta+cmd_sta) + dfy + y_v
shape_fx(4) * sin(theta+cmd_sta) + shape_fy(4) * cos(theta+cmd_sta) + dfy + y_v]; 
updatedRx = [shape_rx(1) * cos(theta) - shape_ry(1) * sin(theta) + drx + x_v
shape_rx(2) * cos(theta) - shape_ry(2) * sin(theta) + drx + x_v
shape_rx(3) * cos(theta) - shape_ry(3) * sin(theta) + drx + x_v
shape_rx(4) * cos(theta) - shape_ry(4) * sin(theta) + drx + x_v];
updatedRy = [shape_rx(1) * sin(theta) + shape_ry(1) * cos(theta) + dry + y_v
shape_rx(2) * sin(theta) + shape_ry(2) * cos(theta) + dry + y_v
shape_rx(3) * sin(theta) + shape_ry(3) * cos(theta) + dry + y_v
shape_rx(4) * sin(theta) + shape_ry(4) * cos(theta) + dry + y_v];



updatedVx = [shape_vx(1) * cos(theta) - shape_vy(1) * sin(theta) + x_v
shape_vx(2) * cos(theta) - shape_vy(2) * sin(theta) + x_v
shape_vx(3) * cos(theta) - shape_vy(3) * sin(theta) + x_v
shape_vx(4) * cos(theta) - shape_vy(4) * sin(theta) + x_v];

updatedVy = [shape_vx(1) * sin(theta) + shape_vy(1) * cos(theta) + y_v
shape_vx(2) * sin(theta) + shape_vy(2) * cos(theta) + y_v
shape_vx(3) * sin(theta) + shape_vy(3) * cos(theta) + y_v
shape_vx(4) * sin(theta) + shape_vy(4) * cos(theta) + y_v];


dfx = cos(theta)*fd(1) - sin(theta)*fd(2);
dfy = sin(theta)*fd(1) + cos(theta)*fd(2);
drx = cos(theta)*rd(1) - sin(theta)*rd(2);
dry = sin(theta)*rd(1) + cos(theta)*rd(2);
...
set(pl4, 'Function',fn_path);%빨간줄
set(pl5, 'Function',Vehicle_heading);%파란줄
set(pl6, 'Xdata', cx1,'Ydata',cy1); %앞에 초록 점
